import { DEBUG_SHOW_NAV, DEBUG_SHOW_TARGET, FIT_HEIGHT_AND_TILE_X } from './config.js';
...
export function render(hero) {
  ...
  if (DEBUG_SHOW_NAV && nav.canvas) {
    ...
    ctx.globalAlpha = 0.3;
    ctx.drawImage(nav.canvas, sx, sy, sw, sh, 0, 0, Math.ceil(sw * xScale), Math.ceil(sh * yScale));
    ctx.globalAlpha = 1.0;
  }
